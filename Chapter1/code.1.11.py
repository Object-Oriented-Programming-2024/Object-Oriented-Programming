...
class TransactionManager:
 ...
 #A method that allows the creation of a transaction
 def create_transaction(self):
     ...
     trans_amount=float(input("What's the amount of the transaction? "\
                               "Please enter a number"))
     if trans_type==TransactionType.EXPENSE:
         trans_amount*=-1
     trans_description=input("What's the transaction description?")
     trans_currency_input=input("What's the transaction"\
     ...
     trans_date_time_input=input("When did the transaction happen (You can type 'now'"\
                                  "or a date/time in this format: YYYY MM DD hh mm ss)")
 ...
 def get_total_amount(self):
     ...
     # Read the amount and then add it to the total, for each transaction
